...
for folder_i = 1:size(list)
    for videonum = 1:videoNUM
        %rmdir('data/output/', 's');
        img_dir = fullfile('F:/git/paper1st/SalBenchmark-master/Data/DataSet3/ImgsFOGR/', list(folder_i), '/');
        videofolder =  videoFiles(videonum+2).name;
        options.infolder = img_dir{1};%fullfile( foldername, 'data', 'input',videofolder );
       ...
       %% generating final saliency 
        for index = 1:nframe-1
            ...
            if(index == 1)
                ofname = sprintf('F:/git/paper1st/SalBenchmark-master/Data/DataSet3/Saliency/%08d_FOGR.png',folder_i-1);
                imwrite(Energy, ofname);
            end
        end
